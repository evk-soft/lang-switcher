...
**Статус на 2026-09-07:** M1 (Windows MVP) в работе. Реализованы ядро, конфиг, порты, инфраструктура Windows-потоков, DPI-манифест, layered-оверлей, Raw Input курсора, три подписки на раскладку (shell, foreground, TSF) с условным фолбэком и адаптер автозапуска. Доставка переключений языка в разных приложениях и визуальные пункты smoke ещё требуют проверки. **Основной бинарник пока только проверяет DPI-awareness и завершает работу**: отрисовка, трей, звук и общий рантайм остаются впереди. Состояние задач — в [плане M1](docs/superpowers/plans/2026-07-03-m1-windows-mvp.md), исправления исходной реализации — в [аудите](docs/research/2026-09-07-implementation-audit.md).
