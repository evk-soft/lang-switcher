...
| 20 | switcher-app — main.rs: сборка всего, end-to-end | ✅ сборка, реальный старт/выход; смена языка и UI ждут приёмки | этот коммит |
| 21 | приёмка M1: smoke-чеклист и замер NFR | 🟨 начата; ручные пункты и бюджет памяти открыты | |
...
- **G. Деградация (ADR-0007).** Отказ записи автозапуска — снять права на запись у `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` (regedit → Разрешения) и щёлкнуть «Автозапуск»: галка отщёлкивается назад, пункт становится неактивным, тултип получает предупреждение, `config.toml` **не изменился**. Отсутствие звукового устройства (пункт 3 задачи 16). Убить `explorer.exe` — shell-hook перезапускается супервизором (в логе `warn!` с attempt и восстановление раскладки после), бюджет не исчерпан.
