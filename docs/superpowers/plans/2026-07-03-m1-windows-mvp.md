...
**Стек:** Rust stable, windows-rs (включая WASAPI), tray-icon + muda, tiny-skia + ab_glyph, serde + toml, crossbeam-channel, tracing.
...
**Продолжение 2026-09-08:** звук переведён с rodio на WASAPI (ADR-0016), ненужный
hover-таймер отключён (ADR-0017), очередь логгера уменьшена. Native smoke и 152 тест
пройдены; CPU существенно снижен, память 16,87 МиБ всё ещё выше цели. Подробности —
в [отчёте](../../research/2026-09-08-idle-fixes.md). Исторические листинги rodio
в задаче 16 заменены в реализации; их нельзя использовать как актуальный интерфейс.
...
| 21 | приёмка M1: smoke-чеклист и замер NFR | 🟨 audio/tray исправлены, CPU снижен; ручная матрица и RSS >15 МБ открыты | продолжение 2026-09-08 |
