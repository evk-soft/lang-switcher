...
20 реальных RU/EN в отдельном окне пока не выполнены: fixture дважды не получила
foreground и остановилась до первого переключения. Ручная попытка также истекла
без активации; ожидается обратная связь. Это не считается PASS
доставки событий. Для повторения есть `windows/examples/layout_driver.rs`.
