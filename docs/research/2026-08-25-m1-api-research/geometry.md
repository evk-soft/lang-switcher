...
- `GetDpiForWindow`: `DPI_AWARENESS_UNAWARE` → 96, `SYSTEM_AWARE` → системный DPI, `PER_MONITOR_AWARE` → «The DPI of the monitor where the window is located»; невалидный `hwnd` → 0. Т. е. `GetDpiForWindow` — только как сверка внутри `WM_DPICHANGED` → https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getdpiforwindow

  > **ИСПРАВЛЕНИЕ 2026-08-25 (задача 10).** Здесь ранее стояло «для геометрии надёжнее `GetDpiForMonitor` (он не зависит от awareness)» — **это неверно**, и утверждение было без ссылки. Страница `GetDpiForMonitor` говорит прямо противоположное (проверено 2026-08-25): *«This API is not DPI aware and should not be used if the calling thread is per-monitor DPI aware»*, *«When you call GetDpiForMonitor, you will receive different DPI values depending on the DPI awareness of the calling application»*, и таблица: `PROCESS_DPI_UNAWARE` → *«96 because the app is unaware of any other scale factors»*, `PROCESS_SYSTEM_DPI_AWARE` → системный DPI, `PROCESS_PER_MONITOR_DPI_AWARE` → *«The actual DPI value set by the user for that display»* → https://learn.microsoft.com/en-us/windows/win32/api/shellscalingapi/nf-shellscalingapi-getdpiformonitor
  >
  > Практические следствия, уже внесённые в код: (1) PMv2 — не «желательно», а **предусловие правильности** `GetDpiForMonitor`, поэтому `OverlayHwnd::new` сам зовёт `dpi::ensure_per_monitor_v2()` и предупреждает, если поздно; (2) рекомендацию Learn использовать вместо него `GetDpiForWindow` выполнить нельзя — он отвечает про монитор, на котором окно **уже** стоит, а `dpi_for` спрашивает про монитор, куда бейдж только собираются поставить; (3) в PMv2-процессе значения действительно настоящие — smoke-прогон читает 144 и 96 на двух мониторах, что совпадает со строкой `PROCESS_PER_MONITOR_DPI_AWARE`.
...
- ~~**Доставка `WM_DPICHANGED` при нашем собственном перемещении окна.**~~ **ОТВЕЧЕНО 2026-08-25, задача 10** (`cargo run -p switcher-windows --example overlay_smoke`, лог в `docs/smoke/m1-windows.md`, секция C). Сообщение приходит и при перемещении, которое делаем мы сами, причём **синхронно, реентрантно внутри вызова `SetWindowPos`**: строка от wndproc стоит раньше строки отчёта из того же `move_to`. Два следствия: (1) дизайн «ждать `WM_DPICHANGED`» отставал бы на кадр, потому что факты монитора к этому моменту уже посчитаны — сравнение `img.dpi != monitor.dpi` остаётся правильным триггером; (2) реентрантность превращает выбор thread-local флага для инвалидации кэша в **требование безопасности**: wndproc исполняется, пока `move_to` держит `&mut self`, поэтому доступ к тому же состоянию через `GWLP_USERDATA` был бы алиасингом живой мутабельной ссылки.
- ~~**Приходит ли `WM_DPICHANGED` окну `WS_EX_LAYERED|WS_EX_TRANSPARENT|WS_EX_NOACTIVATE|WS_EX_TOOLWINDOW` вообще.**~~ **ОТВЕЧЕНО 2026-08-25, задача 10:** приходит (`msg=736`, дважды за прогон — при первом показе и на пересечении границы мониторов). Сценарий «масштаб сменили в Параметрах, пока бейдж стоит неподвижно» тем самым перестаёт быть недоступным; отдельным пунктом чеклиста он всё же остаётся, потому что наблюдался он не в этом прогоне.
- ~~**Потоковая аффинность `MonitorFromPoint` / `MonitorFromWindow` / `GetMonitorInfoW` / `GetDpiForMonitor` / `GetForegroundWindow`.**~~ **ОТВЕЧЕНО 2026-08-25, задача 10** — двумя разными наблюдениями, потому что одного не хватает:

  1. Автотест `overlay::tests::dpi_for_answers_from_a_foreign_thread` (`Arc<Overlay>` в другой поток, оба потока зовут `dpi_for(Fixed)`): вызов **не вешается**, взаимной блокировки с обработкой сообщений оверлея нет, ответы совпадают.
  2. Smoke-прогон на mixed-DPI столе: `dpi_for` читает **144** на одном мониторе и **96** на другом, то есть возвращает настоящие пер-мониторные значения.

  Второе наблюдение обязательно: `GetDpiForMonitor` зависит от awareness (см. ИСПРАВЛЕНИЕ выше), а DPI-неосведомлённому процессу он отвечает 96 для любого монитора — и 96 же является фолбэком адаптера. Значит на DPI-неосведомлённом прогоне (или просто на одном мониторе со 100 %, как в CI) равенство из пункта 1 выполнялось бы вхолостую и отказ на чужом потоке был бы неотличим от успеха. Тест поэтому зовёт `ensure_per_monitor_v2()` и печатает фактическую awareness в сообщении об ошибке, но **не** утверждает её: другой тест в том же бинарнике мог создать окно раньше, после чего сменить режим уже нельзя, и тест падал бы от порядка, выбранного libtest, а не от дефекта.
...
- ~~**Гарантия `dpix == dpiy` у `GetDpiForMonitor`.**~~ **ОТВЕЧЕНО 2026-08-25, задача 10**, прямой цитатой с той же страницы `GetDpiForMonitor`: *«The values of \*dpiX and \*dpiY are identical. You only need to record one of the values to determine the DPI and respond appropriately»*. Раньше это был вывод из `WM_DPICHANGED`; теперь ссылка. Код продолжает логировать расхождение — одна операция сравнения, зато единственный признак, если гарантия когда-нибудь перестанет держаться.
